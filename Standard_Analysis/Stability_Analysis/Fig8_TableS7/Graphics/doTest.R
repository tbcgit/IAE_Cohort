#!/usr/bin/env Rscript
###############
# ./doTest.R $DATA $META "GVHDs,week" "no.pre,yes.pre" "sinfilt" FALSE 0 0 TAXONOMY FALSE TRUE
...
for(i in 1:ncol(mycombn)){
	...
	mytb <- differential.tests(MATRIX=dat[, ind], GROUPS=GROUPS[ind], GROUP1=g1, GROUP2=g2, TEST="wilcox.test",
						PAIRED=mypaired, CUTOFF=1, COLNAME="feature", FILE=paste("wilcox.test", tag, study, sep="."), DESCRIPTION=DESCRIPTION)
