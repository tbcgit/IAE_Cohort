...
doEnrich <- function(mytb, ind, ind.rest, cond, categ.descr){
	...
	for(go in sel.go.un){
		...
		# nº de veces que aparece la categoría anotada en la lista de genes objetivo
		number.in.sel <- c(number.in.sel, q)
		# total de categorías anotadas para la lista de genes objetivo
		total.in.sel <- c(total.in.sel, k)
		# nº de veces que aparece la categoría anotada en el resto de genes (ind.rest)
		number.in.rest <- c(number.in.rest, m-q)
		# total de categorías anotadas para el resto de genes (ind.rest)
						...
						myfitType="parametric", meta=NULL, datnorm=NULL)
{
  ...
  p.values <- apply(MATRIX, MARGIN=1, function(values) 
  {
	...
	if(TEST=="DESeq2.test"){
		...
		FCdeseq <- res$log2FoldChange
		
		# http://bioconductor.org/packages/devel/bioc/vignettes/DESeq2/inst/doc/DESeq2.html
		# In order to test for differential expression, we operate on raw counts and use discrete distributions. However for other downstream analyses
		# (e.g. for visualization or clustering) it might be useful to work with transformed versions of the count data.
		# There are two options, the VST and the rlog (both are equivalent but VST is faster when there are a lot of samples).
		# The point of these two transformations is to remove the dependence of the variance on the mean, particularly the high
		# variance of the logarithm of count data when the mean is low. Both VST and rlog use the experiment-wide trend of variance over mean, in order
		# to transform the data to remove the experiment-wide trend.
		# Es decir, es caso de que el input de doTestDESeq2.R sea una matriz de frecuencias relativas (recordar que el input aconsejable para deseq2 son los
		# absolute counts) estas transformaciones no tienen mucho sentido, ya que ya se habrá eliminado el experiment-wide trend.
		# También hay que tener en cuenta que estas transformaciones están pensadas para datos de RNA-seq (When the expected amount of variance is
		# approximately the same across different mean values, the data is said to be homoskedastic. For RNA-seq counts, however, the expected variance
		# grows with the mean), por lo que en caso de tener datos de DNA quizás sea suficiente abordar los análisis multivariantes con las frecuencias relativas.
	...
	if(TEST=="DESeq3.test"){
		if(is.null(PAIRED)){
			...
			# En este punto PAIRED vale "donor" (caso de estudio pareado) o "cualquier otro nombre" en caso de ser un estudio (diferente de un estudio pareado)
			# con diferentes observaciones de cada individuo, pero el tratamiento en DESeq2 es igual en ambos casos, con efecto fijo.
			coldata <- data.frame(sample=names(GROUPS), condition=as.factor(GROUPS), donor=as.factor(meta[, PAIRED]))
			rownames(coldata) <- names(GROUPS)
			dds <- DESeqDataSetFromMatrix(countData = MATRIX, colData = coldata, design = ~ donor + condition)
		}
		dds <- DESeq(dds, fitType=myfitType)
	...
	if(TEST=="DESeq2.test"|TEST=="DESeq3.test") TEST <- "DESeq2.test"
  ...
  if(TEST=="DESeq2.test"){
  	df <- data.frame(p.value=p.values, adj.p.value=p.values.adj, 
                   mean1=means.group1, mean2=means.group2, log2FC=FC, log2FCdeseq=FCdeseq, stringsAsFactors=FALSE)
  } else {
  	df <- data.frame(p.value=p.values, adj.p.value=p.values.adj, 
  ...
  if(TEST=="DESeq2.test") return(list(df.aux=df.aux, deseqT=deseqT)) else return(df.aux)
}
...
get.colours <- function(GROUPS)
{
  ...
  if(length(groups.unique) > length(mypallete)){
    ...
    indcol <- c(6,10,12,17,26,31,33,43,47,51,52,53,60,61,62,66,68,76,84,85,86,88,101,103,419,116,367,410,463,468)
    indcoltem <- indcol[1:length(groups.unique)]
    mypallete <- colors()[indcoltem]
    if(length(groups.unique)>length(indcol)) stop("You have more than 30 groups, add colours to the pallete.")
  }
...
doSummary <- function(file=NULL, mytb=NULL, mincutoff=0.6, sampleColumn="cluster", groups2plot=NULL,
	...
	if(length(ind.taxa)>0){
		...
		if(mytaxlevel=="family") all.data$family <- unlist(lapply(strsplit(all.data$feature, split=";"), FUN=function(x) x[4]))
		all.data$phylum <- unlist(lapply(strsplit(all.data$feature, split=";"), FUN=function(x) x[1]))
		# Este table tal y como está lleva a error a veces (cuando hay diferentes phylum con family|genus ambos iguales a f.NA|g.NA se colapsan todos al mismo grupo o
		# incluso dentro del mismo phylum también si hay 2 clases u órdenes diferentes con family|genus ambos iguales a f.NA|g.NA se colapsan todos al mismo grupo).
		# Para arreglarlo hemos añadido un trozo de código en doBarplot.R (mirar donde dice *** arreglo ***).
...
mybarplot <- function(myphylum, ta.dfm.pct, mymain, mybottom=5, splitLegend=FALSE){	
	...
	my.unique.phy <- setdiff(my.unique.phy, c("Actinobacteriota", "Bacteroidota", "Firmicutes", "Proteobacteria", "Fusobacteria")) 
...
rel_ancom<-function(OTUdat, Vardat, main.var="country", pr=NULL, pii=NULL,ref_name=NULL, paired=FALSE, test=TRUE, extrafact="NA"){
  ########## STEP A1 ################################################################
  ...
  if(extrafact!="NA"&extrafact!="donor") gauss_data=data.frame(Sample.ID=sid, pop=popu, extrafact=Vardat[, extrafact], redat)
  #############subset only the microbes#######################################
  gauss_microbiome=gauss_data %>% select(-c(Sample.ID,pop))
  if(extrafact!="NA"&extrafact!="donor") gauss_microbiome=gauss_data %>% select(-c(Sample.ID,pop,extrafact))
  ...
  for (j in 1:d){
	...
	if(extrafact!="NA"&extrafact!="donor") regress=data.frame(response,covariate=factor(covariate),covariate2=factor(gauss_data$extrafact))
	regress2=na.omit(regress)
	if(paired==FALSE){
		model=lm(regress2$response~ factor(regress2$covariate))
		if(extrafact!="NA"&extrafact!="donor") model=lm(response~covariate+covariate2, data=regress2)
		anv=anova(model)
		pval[j]=anv$`Pr(>F)`[[1]]
